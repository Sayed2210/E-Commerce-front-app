...
<template>
  <div class="bg-surface-container-lowest rounded shadow-sm p-6">
    <h3 class="font-bold text-on-surface font-headline mb-4">
      {{ $t('admin.dashboardPage.orderStatus') }}
    </h3>
    <div class="space-y-3">
      <div v-for="s in statuses" :key="s.status" class="flex items-center gap-3">
        <div class="w-2 h-2 rounded-full shrink-0" :class="s.color"></div>
        <span class="text-xs text-secondary flex-1 capitalize">{{
          $t(`admin.orderStatus.${s.status}`)
        }}</span>
