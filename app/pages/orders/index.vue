  ...
  <div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="flex gap-1 bg-surface-container-low rounded p-1 mb-8 w-fit">
      <button
        ...
        :class="activeTab === tab.value ? 'bg-surface-container-lowest text-on-surface font-semibold shadow-sm' : 'text-secondary hover:text-on-surface'"
    ...
    <div v-if="pending" class="space-y-4">
      <div v-for="i in 4" :key="i" class="bg-surface-container-lowest rounded p-6 animate-pulse space-y-3">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant/10">
          <div class="flex items-center gap-6">
            <div>
              ...
              <p class="font-bold text-on-surface text-sm font-headline">#{{ order.id.slice(0, 8).toUpperCase() }}</p>
            </div>
            <div>
          ...
          <div class="flex items-center gap-3">
            ...
            <NuxtLink :to="`/orders/${order.id}`" class="text-primary text-xs font-semibold hover:underline flex items-center gap-1">
          ...
          <div class="flex items-center gap-0">
            ...
            >
              <div class="flex flex-col items-center">
                <div
                  :class="stepComplete(order.status, step) ? 'bg-primary-container text-on-primary-container' : 'bg-surface-container text-secondary'"
                  class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors"
                >
                  <span class="material-symbols-outlined text-[12px]" :style="stepComplete(order.status, step) ? `font-variation-settings: 'FILL' 1` : ''">check</span>
                </div>
                <p class="text-[9px] text-secondary mt-1 text-center whitespace-nowrap">{{ step }}</p>
              </div>
              <div
                v-if="i < trackingSteps.length - 1"
                :class="stepComplete(order.status, trackingSteps[i + 1] ?? '') ? 'bg-primary-container' : 'bg-surface-container'"
    ...
    <div v-else class="text-center py-24">
      <span class="material-symbols-outlined text-7xl text-secondary/20 block mb-4">shopping_bag</span>
