<template>
  <div class="max-w-7xl mx-auto px-6 py-4">
    ...
    <nav class="flex items-center gap-2 mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
      <NuxtLink to="/"         class="text-[10px] font-medium tracking-wide uppercase text-secondary hover:text-primary transition-colors">Home</NuxtLink>
      <span class="material-symbols-outlined text-secondary text-[12px]">chevron_right</span>
      <NuxtLink to="/products" class="text-[10px] font-medium tracking-wide uppercase text-secondary hover:text-primary transition-colors">Products</NuxtLink>
      <span class="material-symbols-outlined text-secondary text-[12px]">chevron_right</span>
      <span class="text-[10px] font-medium tracking-wide uppercase text-on-surface">{{ productName }}</span>
    </nav>
    ...
    <div v-else-if="product" class="grid grid-cols-1 lg:grid-cols-12 gap-10">
      ...
      <div class="lg:col-span-5 space-y-4">
        ...
        <div class="grid grid-cols-4 gap-4" v-if="product.images?.length > 1">
          <button
            ...
            :class="activeImage === img ? 'ring-2 ring-primary-container' : 'opacity-60 hover:opacity-100'"
      ...
      <div class="lg:col-span-4 space-y-6">
        <div class="space-y-2">
          <p v-if="product.brand?.name" class="text-xs font-bold uppercase tracking-widest text-secondary">{{ product.brand.name }}</p>
          <h1 class="text-3xl font-extrabold tracking-tight text-on-surface leading-tight font-headline">{{ productName }}</h1>
          <!-- Stars -->
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-secondary">{{ product.averageRating?.toFixed(1) ?? '0.0' }} ({{ product.reviewCount ?? 0 }} reviews)</span>
          </div>
        </div>

        <!-- Price -->
        <div class="flex items-baseline gap-3">
          <span class="text-3xl font-bold text-on-surface font-headline">${{ effectivePrice.toFixed(2) }}</span>
          <span v-if="selectedVariant?.priceModifier && selectedVariant.priceModifier !== 0" class="text-sm text-secondary line-through">${{ product.basePrice.toFixed(2) }}</span>
        </div>
        ...
        <div v-if="product.variants?.length" class="space-y-4">
          <div v-for="(opts, attr) in groupedOptions" :key="attr">
            <p class="text-xs font-bold uppercase tracking-widest text-on-surface mb-2">{{ attr }}</p>
            <div class="flex flex-wrap gap-2">
              <button
                v-for="val in opts"
                :key="val"
                @click="selectOption(attr, val)"
                :class="selectedOptions[attr] === val ? 'bg-on-surface text-surface-container-lowest' : 'bg-surface-container text-secondary hover:bg-surface-container-high'"
        ...
        <div class="space-y-3">
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center border border-outline-variant/30 rounded">
              <button @click="qty = Math.max(1, qty - 1)" class="px-3 py-2 text-secondary hover:text-primary transition-colors">
                <span class="material-symbols-outlined text-sm">remove</span>
              </button>
              <span class="px-4 text-sm font-bold text-on-surface">{{ qty }}</span>
              <button @click="qty++" class="px-3 py-2 text-secondary hover:text-primary transition-colors">
          ...
          <div class="flex gap-3">
            ...
            <button
              ...
              :class="wishlisted ? 'bg-error-container text-on-error-container' : 'bg-surface-container text-secondary hover:text-primary'"
              class="p-3 rounded transition-colors"
            >
              <span class="material-symbols-outlined text-sm" :style="wishlisted ? `font-variation-settings: 'FILL' 1` : ''">favorite</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="selectedVariant" class="flex flex-wrap gap-2">
          <span v-for="(val, key) in selectedVariant.optionValues" :key="key" class="micro-chip bg-surface-container-low text-secondary">
            {{ key }}: {{ val }}
          </span>
          <span v-if="selectedVariant.sku" class="micro-chip bg-surface-container-low text-secondary">SKU: {{ selectedVariant.sku }}</span>
        </div>
      </div>
      ...
      <div class="lg:col-span-3">
        <div class="bg-surface-container-lowest rounded p-6 space-y-4 sticky top-24">
          <p class="text-2xl font-bold text-on-surface font-headline">${{ effectivePrice.toFixed(2) }}</p>
          ...
          <div class="text-xs text-secondary space-y-1 pt-2 border-t border-outline-variant/15">
            <p class="flex items-center gap-2"><span class="material-symbols-outlined text-sm">lock</span> Secure checkout</p>
            <p class="flex items-center gap-2"><span class="material-symbols-outlined text-sm">autorenew</span> 30-day returns</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <section v-if="product" class="mt-16 border-t border-outline-variant/15 pt-12">
      ...
      <div v-if="showReviewForm" class="bg-surface-container-lowest rounded p-6 mb-8 space-y-4">
        ...
        <div class="flex gap-2">
          <button v-for="s in 5" :key="s" @click="newReview.rating = s" class="material-symbols-outlined text-2xl transition-colors"
            :class="s <= newReview.rating ? 'text-primary-container' : 'text-outline-variant'"
            :style="s <= newReview.rating ? `font-variation-settings: 'FILL' 1` : ''">star</button>
        </div>
        <input v-model="newReview.title" type="text" placeholder="Review title" class="w-full bg-surface-container-low border-none rounded py-2 px-4 text-sm outline-none focus:ring-1 focus:ring-primary" />
        <textarea v-model="newReview.comment" placeholder="Share your experience…" rows="4" class="w-full bg-surface-container-low border-none rounded py-2 px-4 text-sm outline-none focus:ring-1 focus:ring-primary resize-none"></textarea>
        <div class="flex gap-3">
          <button @click="submitReview" class="bg-primary-container text-on-primary-container px-6 py-2 rounded text-sm font-semibold hover:bg-primary hover:text-on-primary transition-all">
            Submit
          </button>
          <button @click="showReviewForm = false" class="text-sm text-secondary hover:text-on-surface transition-colors">Cancel</button>
        </div>
      </div>

      <!-- Reviews list -->
      <div v-if="reviewsPending" class="space-y-4">
        <div v-for="i in 3" :key="i" class="bg-surface-container-lowest rounded p-6 animate-pulse space-y-2">
      ...
      <div v-else-if="reviews.length" class="space-y-4">
        <div v-for="review in reviews" :key="review.id" class="bg-surface-container-lowest rounded p-6">
          <div class="flex items-start justify-between mb-3">
            <div>
              <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="review.isVerifiedPurchase" class="micro-chip bg-surface-container-low text-secondary">Verified</span>
              </div>
          ...
          <p v-if="review.user" class="text-xs text-secondary mt-3">— {{ review.user.firstName ?? review.user.email }}</p>
        </div>
      </div>
      <p v-else class="text-secondary text-sm">No reviews yet. Be the first to review this product!</p>
    </section>
  </div>
</template>
...
const { data: reviewsData, pending: reviewsPending, refresh: refreshReviews } = await getProductReviews(id)
const reviews = computed<Review[]>(() => (reviewsData.value as any) ?? [])

// Gallery
const activeImage = ref<string>('')
watch(product, (p) => { if (p?.images?.[0]) activeImage.value = p.images[0] }, { immediate: true })
