  ...
  <div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex items-end justify-between mb-8">
      <div>
        ...
        <p class="text-secondary text-sm mt-1">{{ items.length }} saved item{{ items.length !== 1 ? 's' : '' }}</p>
      </div>
      <button
        v-if="items.length"
        @click="clearAll"
    ...
    <div v-else-if="items.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      >
        ...
        <div class="p-4 flex flex-col flex-1">
          ...
          <NuxtLink :to="`/products/${item.productId}`" class="font-bold text-sm text-on-surface leading-tight mb-2 line-clamp-2 hover:text-primary transition-colors flex-1">
            {{ productName(item.product) }}
          </NuxtLink>
          <div class="flex items-center gap-1 mb-3">
            <span v-for="s in 5" :key="s" class="material-symbols-outlined text-[10px]"
              :class="s <= Math.round(item.product?.averageRating ?? 0) ? 'text-primary-container' : 'text-outline-variant'"
              :style="s <= Math.round(item.product?.averageRating ?? 0) ? `font-variation-settings: 'FILL' 1` : ''">star</span>
            <span class="text-[10px] text-secondary ml-1">{{ item.product?.reviewCount ?? 0 }}</span>
          </div>
          <div class="flex items-center justify-between mt-auto">
            <span class="text-primary font-bold text-lg">${{ item.product?.basePrice?.toFixed(2) ?? '—' }}</span>
            <button
        ...
        <!-- Added date -->
        <div class="px-4 pb-3 text-[10px] text-secondary">
    ...
    <div v-else class="text-center py-24">
      <span class="material-symbols-outlined text-7xl text-secondary/20 block mb-4" style="font-variation-settings: 'FILL' 0">favorite_border</span>
