<template>
  <div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div v-if="isEmpty && !loading" class="text-center py-24">
      <span class="material-symbols-outlined text-7xl text-secondary/20 block mb-4">shopping_cart</span>
      ...
      <NuxtLink to="/products" class="inline-block bg-primary-container text-on-primary-container px-8 py-3 rounded font-bold text-sm hover:bg-primary hover:text-on-primary transition-all">
        Start Shopping
      </NuxtLink>
    </div>
    ...
    <div v-else class="flex flex-col lg:flex-row gap-8">
      ...
      <section class="lg:w-3/4 space-y-4">
        ...
        <div v-if="loading" class="space-y-4">
          <div v-for="i in 3" :key="i" class="bg-surface-container-lowest rounded p-6 flex gap-6 animate-pulse">
          ...
          <div class="flex-grow flex flex-col justify-between">
            <div>
              <div class="flex justify-between items-start">
                <div>
                  <h3 class="font-headline font-semibold text-lg text-on-surface">{{ productName(item.product) }}</h3>
                  <p v-if="item.variant" class="text-secondary text-sm mt-0.5">
                    Variant: {{ variantName(item.variant) }}
                  </p>
                </div>
                <p class="font-headline font-bold text-xl text-on-surface">${{ item.totalPrice.toFixed(2) }}</p>
              </div>
              <div class="mt-2">
                <span
                  :class="item.product?.inventoryQuantity > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-error-container text-on-error-container'"
            ...
            <div class="flex items-center gap-6 mt-4">
              <div class="flex items-center gap-2">
                ...
                <div class="flex items-center border border-outline-variant/20 rounded">
                  <button @click="changeQty(item, item.quantity - 1)" class="px-2 py-1 text-secondary hover:text-primary transition-colors">
              ...
              <div class="flex gap-4">
                <button @click="remove(item.id)" class="text-xs text-error hover:underline transition-all">Remove</button>
                <button @click="saveForLater(item.id)" class="text-xs text-primary hover:underline transition-all">Save for later</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Continue shopping -->
        <NuxtLink to="/products" class="inline-flex items-center gap-2 text-primary text-sm font-semibold hover:underline mt-2">
      ...
      <aside class="lg:w-1/4 space-y-4">
        <div class="bg-surface-container-lowest rounded p-6 space-y-4 sticky top-24">
          <h2 class="font-headline font-bold text-lg text-on-surface border-b border-outline-variant/15 pb-3">Order Summary</h2>

          <!-- Coupon -->
          <div class="space-y-2">
            <label class="text-xs font-bold uppercase tracking-widest text-secondary">Coupon Code</label>
            <div class="flex gap-2">
              ...
              />
              <button @click="applyCoupon" class="bg-surface-container text-primary px-3 py-2 rounded text-xs font-bold hover:bg-primary-container hover:text-on-primary-container transition-all">
                Apply
              </button>
            </div>
            <p v-if="couponMessage" class="text-xs" :class="couponApplied ? 'text-green-600' : 'text-error'">{{ couponMessage }}</p>
          </div>
          ...
          <div class="space-y-2 text-sm border-t border-outline-variant/15 pt-4">
            ...
            <div class="flex justify-between text-secondary">
              <span>Shipping</span>
              <span class="text-green-600 font-semibold">{{ subtotal >= 99 ? 'FREE' : '$9.99' }}</span>
            </div>
            <div class="flex justify-between font-bold text-on-surface text-base border-t border-outline-variant/15 pt-2 mt-2">
              <span>Total</span>
          ...
          <div class="flex items-center justify-center gap-4 text-xs text-secondary pt-2">
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">credit_card</span> Stripe</span>
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">account_balance</span> PayPal</span>
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">payments</span> COD</span>
          </div>
        </div>
      </aside>
    </div>
